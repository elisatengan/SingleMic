...
n = 32
f, Pxx_den_32 = scipy.signal.periodogram(y[0:n], fs=fs, return_onesided=False)
...
n = 64
f, Pxx_den_64 = scipy.signal.periodogram(y[0:n], fs=fs, return_onesided=False)
...
n = 128
f, Pxx_den_128 = scipy.signal.periodogram(y[0:n], fs=fs, return_onesided=False)
...
n = 256
f, Pxx_den_256 = scipy.signal.periodogram(y[0:n], fs=fs, return_onesided=False)
...
n = 512
f, Pxx_den_512 = scipy.signal.periodogram(y[0:n], fs=fs, return_onesided=False)
plt.plot(f, Pxx_den_512)
plt.xlabel('frequency [Hz]')
plt.ylabel('PSD [V**2/Hz]')
plt.title("nperseg = 512")


n = 1024
f, Pxx_den_1024 = scipy.signal.periodogram(y[0:n], fs=fs, return_onesided=False)
plt.plot(f, Pxx_den_1024)
plt.xlabel('frequency [Hz]')
plt.ylabel('PSD [V**2/Hz]')
plt.title("nperseg = 1024")
